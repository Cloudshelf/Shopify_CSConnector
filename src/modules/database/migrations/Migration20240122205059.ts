...
export class Migration20240122205059 extends Migration {
    async up(): Promise<void> {
        ...
        this.addSql(
            'alter table "noble_task_error" add constraint "noble_task_error_task_id_foreign" foreign key ("task_id") references "noble_task" ("id") on update cascade on delete cascade;',
        );

        this.addSql(
            'alter table "noble_task_log" add constraint "noble_task_log_task_id_foreign" foreign key ("task_id") references "noble_task" ("id") on update cascade on delete cascade;',
        );
    }
